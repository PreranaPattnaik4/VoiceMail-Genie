...
- "draft": Creates an initial email draft from a goal. The 'goal' in the arguments should be the original user goal, potentially rephrased for clarity. This is a mandatory first step.
...
Analyze the user's goal and generate a logical sequence of steps. The plan MUST start with a "draft" step and MUST end with a "proofread" step.
- For "Email my manager to thank them for the meeting in a formal tone," the plan should be [draft, tone, proofread].
- For "send an email in Spanish to a client about a deadline extension", the plan should be [draft, translate, proofread].
- For "write a quick note to my team about the new update", the plan is just [draft, proofread].
